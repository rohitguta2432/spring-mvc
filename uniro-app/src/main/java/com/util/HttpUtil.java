...
public class HttpUtil {
	...
	private final String uniroRestBaseUrl;

	@Autowired
	public HttpUtil(@Value("${uniro.rest.base.url}") String uniroRestBaseUrl) {
		this.uniroRestBaseUrl = uniroRestBaseUrl;
	}
	...
	public String postOrPut(String url, Object payloadData, Map<String, String> headers, HttpMethod method) {
		...
		String finalUrl = uniroRestBaseUrl + url;
		...
		try {
			...
			throw new UniroWebException("Error while making request to url");
		}
		...
		try {
			...
			throw new UniroWebException("Error while obtaining response from url");
		}

		if (response == null) {
			log.error(String.format("No response obtained for url"));
			throw new UniroWebException("No response obtained for url request");
		}
	...
	private String parseHttpResponse(HttpResponse response, String url) {
		...
		} catch (UniroWebException | UnsupportedOperationException | IOException e) {
